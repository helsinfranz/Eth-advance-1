...
export default function org_regiatration_page() {
  ...
  const [Organization_access_level, setOrganization_access_level] =
    useState(undefined);
  const [Organization_Token_name, setOrganization_Token_name] =
    useState(undefined);
  const [Organization_token_abbreviation, setOrganization_token_abbreviation] =
    useState(undefined);
  const [
    Organization_token_totall_supply,
    setOrganization_token_totall_supply,
  ] = useState(undefined);
  ...
  const organization_registrationHandler = async () => {
    ...
    if (smartcontract) {
      ...
      console.log(
        `here is the output of user account address msg ${value} user account Organization_token_total_supply ${ethers.BigNumber.from(
          Organization_token_totall_supply
        )} `
      );
      const value1 = await smartcontract._mint(
        account,
        Organization_name,
        Organization_Token_name,
        Organization_token_abbreviation,
        ethers.BigNumber.from(Organization_token_totall_supply)
      );
  ...
  const InfoCheck = async () => {
    ...
    if (smartcontract) {
      const [
        ...
        Organization_token_totall_supply,
        adminBalance,
      ] = await smartcontract._show_organization_details(account);
      ...
      setOrganization_token_totall_supply(
        parseInt(Organization_token_totall_supply)
      );
    }
    return 1;
  };
      ...
      return (
        <>
          <div>
            ...
            <p>
              Organization Token Abbreviationis ={" "}
              {Organization_token_abbreviation}
            </p>
            <p>
              Organization Token Totall Supply is ={" "}
              {Organization_token_totall_supply}
            </p>
          </div>
        </>
  ...
  const walletconnectbutton = () => {
    if (account == undefined) {
      ...
      return (
        <>
          <div>
            Click on Check to see Your Organization details associated with
            Current Account =
  ...
  return (
    <>
      <div>
        ...
        <br></br>
        <div>
          <button onClick={() => organization_registrationHandler()}>
